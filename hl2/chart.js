/* ═══════════════════════════════════════════════════════════════
   HL Trade · chart.js v3.0 — Final
   ✅ Lightweight Charts v4 — بدون Volume
   ✅ touch-action:none — pinch داخل الرسم فقط
   ✅ منع gesturestart/wheel+ctrl من تكبير المتصفح
   ✅ Fullscreen عند الفتح
   ✅ ساعة UTC+3 وسط الشاشة
   ✅ أزرار شراء/بيع كبيرة + حقل كمية واضح
...
.c-nav {
  ...
  flex-shrink:0; position:relative; direction:rtl;
}
.c-back {
...
.c-ws { font-size:12px; flex-shrink:0; }
...
/* أزرار بيع / شراء — كبيرة جداً لسهولة النقر */
.cbt-btn {
...
.cbt-qty-row { display:flex; align-items:center; gap:4px; width:100%; }
.cbt-qty-in {
...
`;
  ...
  function startClock() {
    ...
    const tick = () => {
      ...
      const ap   = now.getUTCHours()<12 ? 'ص' : 'م';
      const dd   = String(now.getUTCDate()).padStart(2,'0');
      const mo   = String(now.getUTCMonth()+1).padStart(2,'0');
      const yr   = now.getUTCFullYear();
      const t    = document.getElementById('_cClockT');
      const d    = document.getElementById('_cClockD');
      if (t) t.textContent = `${hh}:${mm}:${ss} ${ap}`;
      if (d) d.textContent = `${dd}-${mo}-${yr} · UTC+3`;
    };
    tick();
  ...
  function showCf(isBuy) {
    ...
    document.getElementById('_cfX').onclick = () => execTrade(isBuy, qty);
  }
  ...
  function buildChart(container) {
    ...
    _chart = LightweightCharts.createChart(container, {
      ...
      rightPriceScale:{ borderColor:BDR, scaleMargins:{top:.06,bottom:.06}, minimumWidth:72 },
      timeScale:{ borderColor:BDR, timeVisible:true, secondsVisible:false, rightOffset:8, barSpacing:8, lockVisibleTimeRangeOnResize:true },
      handleScroll:{ mouseWheel:true, pressedMouseMove:true, horzTouchDrag:true, vertTouchDrag:false },
      handleScale:{ mouseWheel:true, pinch:true, axisPressedMouseMove:{time:true,price:false} },
      localization:{ locale:'en-US', priceFormatter:p=>p.toLocaleString('en-US',{minimumFractionDigits:ai(_sym).pxDp,maximumFractionDigits:ai(_sym).pxDp}) },
  ...
  function drawLines(){
    ...
    for(const p of (State.positions||[])){
      ...
      const ts=p.tpsl||{};
      if(ts.tp) _tpLine=_series.createPriceLine({price:ts.tp,lineWidth:1,lineStyle:3,color:'#22c58b',axisLabelVisible:true,title:'TP'});
      if(ts.sl) _slLine=_series.createPriceLine({price:ts.sl,lineWidth:1,lineStyle:3,color:'#e8804a',axisLabelVisible:true,title:'SL'});
      break;
    }
  }
  ...
  function ensureScreen(){
    ...
    screen.innerHTML=`
      <div class="c-nav">
        ...
        <div class="c-clock">
          <span class="c-clock-time" id="_cClockT">—</span>
          <span class="c-clock-date" id="_cClockD">—</span>
        </div>
        <span class="c-ws" id="_cWs">⏳</span>
      </div>
      <div class="c-intervals">
  ...
  function close(){
    _visible=false;
    wsClose(); hideCf(); stopClock(); exitFull();
